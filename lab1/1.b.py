...
N = 10
EPSILON = 0.5
MEAN = 1  # мю - математическое ожидание
STD_DEV = EPSILON / 3 # сигма - стандартное отклонение, учитываем правило трех сигм

def generate_x():
    return np.random.uniform(-X, X)

def generate_eps():
    eps = np.random.normal(MEAN, STD_DEV, 1)
    bounded_eps = np.clip(eps, -EPSILON, EPSILON)
    return bounded_eps
...
def draw_function():
    ...
    plt.plot(x, y, label="x*sin(2*Pi*x)")  
    plt.title('График функции и выборки') 
    plt.xlabel('x') 
    plt.ylabel('f(x)')
    plt.text(-2.5, -5.5, f'N={N}, eps0={EPSILON}, mean={MEAN}, std_dev={STD_DEV}', fontsize=8, color='blue')   
    plt.grid(True)  
    plt.legend() 
    plt.xlim(-3, 3) 
    plt.ylim(-6, 6) 
...
def main():
    ...
    plt.scatter(x, y, color='red', label='Выборка')  
    x_true = np.linspace(-5, 5, 200)
    plt.plot(x_true, f(x_true), color='black', linestyle='--', label='Истинная функция', alpha=0.7)
